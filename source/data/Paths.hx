...
class Paths
{
	...
	public static var dumpExclusions:Array<String> = [
		'assets/music/futuremenu.$SOUND_EXT',
		'assets/shared/music/layback.$SOUND_EXT',
		'assets/shared/music/breakfast.$SOUND_EXT',
		'assets/shared/music/tea-time.$SOUND_EXT',
	];

	inline public static function cacheSound(key:String)
		return OpenFlAssets.getSound(key);

	public static function clearSoundCache() {
		for (k in cast(OpenFlAssets.cache, openfl.utils.AssetCache).sound.keys()) {
			if (!dumpExclusions.contains(k)) {
				OpenFlAssets.cache.removeSound(k);
			}
		}
	}
	...
	{
		var songKey:String = '${formatToSongPath(song)}/Voices-Player'; // jk fuck you fag
		var voices = returnSound('songs', songKey);
		return voices;
	}

	inline static public function secVoices(song:String):Any
	{
	...
	public static function returnGraphic(key:String, ?library:String)
	{
		...
		if (OpenFlAssets.exists(path, IMAGE)) {
			if(!currentTrackedAssets.exists(path)) {
				if (!ClientPrefs.gpuCache){ // sorry ziad but dw its disabled by defauled
					var newGraphic:FlxGraphic = FlxG.bitmap.add(path, false, path);
					newGraphic.persist = true;
					currentTrackedAssets.set(path, newGraphic);
				}else{
					var newGraphic:FlxGraphic = FlxGraphic.fromAssetKey(path, false, path);
					var bitmap = newGraphic.bitmap;
					if (bitmap != null) {
						...
						newGraphic.dump();
						newGraphic.bitmap.dispose();
						newGraphic.bitmap.disposeImage();
						newGraphic.bitmap = bitmap;
					}
					newGraphic.persist = true;
					currentTrackedAssets.set(path, newGraphic);
				}
