...
void main()
{
	...
	signal += pass1(uv - vec2(0.1 / openfl_TextureSize.x, 0.0)).xyzw *
...
')
	...
	public function new() {
		...
		this.uFrame.value = [0];
		this.uInterlace.value = [0.6];
	}

	public var interlace(get, set):Bool;

	function get_interlace() {
		return this.uInterlace.value[0] == 0.6;
	}
	function set_interlace(val:Bool) {
		this.uInterlace.value[0] = val ? 0.6 : 0.0;
		return val;
	}
