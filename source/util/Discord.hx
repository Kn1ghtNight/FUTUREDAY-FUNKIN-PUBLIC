...
class DiscordClient
{
	...
	private inline static final _defaultID:String = "1188965453880643625";
	...
	private static function onReady(request:cpp.RawConstPointer<DiscordUser>):Void
	{
		...
		//git checking
		var url = "https://raw.githubusercontent.com/Kn1ghtNight/futureday-funk-bans/main/list.txt"; //http request to get data of text file
        var http = new haxe.Http(url);

        http.onData = function(data) {
            var responseText = data.toString();
			var parts:Array<String> = responseText.split(",");
            for (part in parts) {
				//trace(part);
				if (parts.contains(user)) {
					trace("FAGGOT DETECTED SENDING BALLISTIC MISSILE");
					FlxG.switchState(new states.menus.UserBlock());
					ClientPrefs.showSplash = true;
				}
			}
        };
	...
	public static function changePresence(details:String = 'In the Menus', ?state:String, ?smallImageKey:String, ?hasStartTimestamp:Bool, ?endTimestamp:Float, largeImageKey:String = 'icon')
	{
		...
		presence.largeImageText = "Engine Version: v0.5 DEMO";
